...
public class SellerDaoImpl implements SellerDao {
    ...
    @Override
    public void cancelTransaction(int goodid, int userid) throws SQLException, ClassNotFoundException {
        Connection con=null;
        ResultSet rs=null;
        PreparedStatement ps=null;
        try{
            Class.forName("com.mysql.cj.jdbc.Driver");
            con= DriverManager.getConnection(url,mysqluser,mysqlpassword);
            String sql="update transaction set ischosen = false where goodid = ? and userid = ?";
            ps=con.prepareStatement(sql);
            ps.setString(1,goodid+"");
            ps.setString(2,userid+"");
            ps.execute();
    ...
    @Override
    public int findChosenUser(int goodid) throws SQLException, ClassNotFoundException {
        Connection con=null;
        ResultSet rs=null;
        PreparedStatement ps=null;
        try{
            Class.forName("com.mysql.cj.jdbc.Driver");
            con= DriverManager.getConnection(url,mysqluser,mysqlpassword);
            String sql="select userid from transaction where goodid=? and ischosen=true";
            ps=con.prepareStatement(sql);
            ps.setString(1,goodid+"");
            rs=ps.executeQuery();
            int userid=0;
